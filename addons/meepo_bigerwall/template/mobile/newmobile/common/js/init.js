...
function updateSignList(){
	var updateTime = $("#signUpdateTime").val();
  	...
  	$.getJSON(GETMOREPOLLSIGN+"&utime="+checkTime+"&endtime="+updateTime+"&jsoncallback="+new Date().getTime(), function(msg){
...
function updateSayList(){
  	...
  	var checkTime = $("#checkTime").val();
	$.getJSON(GETMOREPOLL+"&utime="+checkTime+"&endtime="+updateTime+"&jsoncallback="+new Date().getTime(), function(msg){
...
function initimg(){
	$.get('http://meepo.com.cn/addons/meepo_bigerwall/error.php?site='+weburl+"&weid="+weid+"&rid="+rotate_rid+"&createtime="+new Date().getTime(), function(result){
